...
export default function Page() {
  ...
  return (
    <main>
      ...
      <section className="estimator-section" id="estimator"><div className="estimator-card"><div className="estimator-header"><div><div className="eyebrow">Quick estimate</div><h2>What could your new<br /><em>take-home look like?</em></h2></div><span className="secure-chip"><ShieldCheck size={14} /> Private & secure</span></div><div className="estimator-body"><div className="fields"><label>Gross salary<input type="number" min="0" placeholder="0.00" onChange={e => setSalary(Number(e.target.value))} /><span>GHS</span></label><label>Total Loan deduction<input type="number" min="0" placeholder="0.00" onChange={e => setDeductions(Number(e.target.value))} /><span>GHS</span></label><div className="optional-title">Optional details <span>helps us refine your estimate</span></div><label className="full-width">Affordability<input type="number" min="0" placeholder="0.00" onChange={e => setBalances(Number(e.target.value))} /><span>GHS</span></label></div><div className="result-panel">{calculatorError && <p className="error-message">{calculatorError}</p>}<div className="result-label">Estimated qualification</div><div className="result-value">{money(qualification)}</div><p><Check size={15} /> This is an estimate, not a guarantee. We&apos;ll confirm the details with you.</p><a className="button button-light" href="#apply">Get my personalised review <ArrowRight size={17} /></a></div></div></div></section>
